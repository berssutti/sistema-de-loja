 ...
 * Integer.parseInt(dadosFuncionario[0])); return true; } } // a posicao 0 do
 * vetor dadosAlunos indica onde os dados devem ser inseridos public boolean
 * inserirEditarCliente(String[] dadosCliente) {
 * if(!dadosCliente[2].matches("[0-9]+") || !dadosCliente[3].matches("[0-9]+")
 * || !dadosCliente[4].matches("[0-9]+")) { return false; } else { //Aluno a =
 * new Aluno(dadosCliente[1], dadosCliente[2],
 ...
 * Integer.parseInt(dadosCliente[0])); return true; } }
 * 
 * public boolean removerCliente(int i) { String clienteRemovido =
 * dados.getCliente()[i].getNome(); String aux;
 * 
 * if(i == (dados.getQtdClientes() - 1)) { // O aluno a ser removido está no
 ...
 * 
 * 
 * public ControlDados() { dados.addDados(); }
 * 
